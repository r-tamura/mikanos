...
struct SyscallResult SyscallOpenWindow(int w, int h, int x, int y, const char* tilte);

#define LAYER_NO_REDRAW (0x00000001ull << 32)
struct SyscallResult SyscallWinWriteString(
    uint64_t layer_id, int x, int y, uint32_t color, const char* s);
struct SyscallResult SyscallWinFillRectangle(
    uint64_t layer_id, int x, int y, int w, int h, uint32_t color);
...
struct SyscallResult SyscallWinDrawLine(
    uint64_t layer_id, int x0, int y0, int x1, int y1, uint32_t color);
